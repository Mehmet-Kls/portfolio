          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {projects.map((p, i) => (
              <Reveal key={p.id} direction="up" delay={i * 100}>
                <article className="group relative flex flex-col border border-[var(--line)] bg-[var(--bg-surface)] p-6 transition-all duration-300 hover:border-[var(--accent-volt-dim)] hover:-translate-y-1 h-full">
                  {p.featured && (
                  ...
                  {(p.projectUrl || p.githubUrl) && (
                    <div className="mt-5 flex gap-4 mono text-xs">
                      {p.projectUrl && (
                        <a href={p.projectUrl} target="_blank" rel="noopener noreferrer" className="text-[var(--accent-volt)] hover:text-glow">
                          → görüntüle
                        </a>
                      )}
                      {p.githubUrl && (
                        <a href={p.githubUrl} target="_blank" rel="noopener noreferrer" className="text-[var(--text-secondary)] hover:text-[var(--text-primary)]">
                          ⌥ kaynak kod
                        </a>
                      )}
                    </div>
